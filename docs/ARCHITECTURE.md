...
`ILocalizationService` owns presentation culture and embedded `en-US`/`pt-BR` resource dictionaries. Views bind user-facing labels through dynamic application resources, while ViewModels request messages through the same service. Navigation uses stable IDs instead of translated titles. Protocol identifiers, HEX, enum values persisted in JSON, capture operations, timestamps written to files, and `.mdbcap.json` schema fields remain culture invariant.
