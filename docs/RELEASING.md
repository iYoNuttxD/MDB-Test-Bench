...
SHA256SUMS.txt (generated by the release workflow over the four final archives)
```
...
3. Run `MDB-Test-Bench --smoke-test` and `MDB-Test-Bench --discovery-smoke-test`; require exit code zero from both.
...
`.github/workflows/release.yml` runs for tags matching `v*` and supports a controlled `workflow_dispatch` with a required `version` input. Manual dispatch requires the corresponding immutable tag to exist. It validates SemVer and the source `VersionPrefix`, refuses to overwrite an existing release, then builds/tests and packages each target on an appropriate official GitHub-hosted runner. The final job verifies the four expected archives, creates `SHA256SUMS.txt`, and uses the GitHub CLI plus the workflow token to publish all five assets with bilingual notes. Job permissions default to read-only; only the final publication job receives `contents: write`.

To publish after review:

```bash
git tag -a v0.1.1 -m "MDB Test Bench v0.1.1"
...
If a release is defective, do not move or overwrite an existing semantic-version tag. Mark the GitHub Release as a prerelease or remove only the GitHub Release through an authorized maintainer action, document the reason, fix the defect, and publish a new patch version such as `v0.1.2`. Removing a remote tag is exceptional and requires coordination because consumers may already have fetched it.
